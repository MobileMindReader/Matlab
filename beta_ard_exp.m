...
%         model.w = zeros(1, numFuncs);
        ...
        x_sparse_test=zeros(size(A,2), 1);
        for i=1:size(A,2)
            xInput = sin(0.5*randn);
            x_test(i,:)=model.w(i)*xInput;
            xSparseTest(i,:)=model.wSparse(i)*xInput;
        end
        
        data.w_true{iter, intraIter} = x;
        data.w_trueSparse{iter, intraIter} = xSparse;
        data.w_trueSparseTest{iter, intraIter} = xSparseTest;
        
%         data.w_test = x_test;
        data.x = x;
        data.x_sparse = xSparse;
        data.x_test = x_test;
        data.x_test_sparse = xSparseTest;
        
        y = A*x;
        y_test = A*x_test;
        
        ySparse = A*xSparse;
        y_test_sparse = A*xSparseTest;
        ...
        targetsSparseTest = y_test_sparse + normrnd(0, sqrt(1/model.beta), [N 1]);
...
%         data.w_ard_sparse_norm_fixed(iter, intraIter) = norm(w_ard_sparse_fixed);
        ...
        data.error(iter, intraIter) = mean((w_ard(:) - data.x(:)).^2);
        
        Sigma = inv(diag(A)+(beta*(Phi'*Phi)));
        w_ard_test = betaSparse*Sigma*Phi'*targets_test;
        
        data.error_test(iter, intraIter) = mean((w_ard_test(:) - data.x_test(:)).^2);
        
        
        data.errorSparse(iter, intraIter) = mean((w_ard_sparse(:) - data.x_sparse(:)).^2);
        
        SigmaSparse = inv(diag(ASparse)+(betaSparse*(Phi'*Phi)));
        w_ard_sparse_test = betaSparse*SigmaSparse*Phi'*targetsSparseTest;
        
        data.error_test_sparse(iter, intraIter) = mean((w_ard_sparse_test(:) - data.x_test_sparse(:)).^2);        
...
plot(mean(data.w_true_sparse_norm,2));
...
plot(mean(data.errorSparse,2)./normalizer);
plot(mean(data.error_test,2)./normalizer); 
plot(mean(data.error_test_sparse,2)./normalizer);
% plot(mean(data.error_fixed,2));
% plot(mean(data.errorSparse_fixed,2));
% plot(mean(data.error_ridge1,2)); 
hold off;
legend('Train dense', 'Train sparse', 'Test dense', 'Test sparse');
...
%%
normalizer=[10:10:400]';

normalizedEr = (mean(data.error,2)./normalizer);
normalizedErTest = (mean(data.error,2)/normalizer);
normalizedErSp = (mean(data.error,2)/normalizer);
normalizedErSpTest = (mean(data.error,2)/normalizer);

figure(3)
% subplot(3,1,3);
plot(normalizedEr); hold on;
plot(mean(data.error_test,2)/normalizer); 
...
hold off;
legend('Train dense', 'Test dense', 'Train sparse', 'Test sparse');
