...
figure(5)
% errorbar(mean(alpha_uni,2), error_shared);
% errorbar(squeeze(mean(active_alphas,2)), error_separate');
% errorbar(squeeze(mean(active_alphas(:,:,idx),2))', error_separate(idx,:));
errorbar(1:50, mean(alpha_std_separate,2), mean(error_separate,1));
meanSTD = std(alpha_std_separate,2);

set(gca,'XTick',ticks);
set(gca,'XTickLabel',tickLabels);
title('alpha estimation with error bars (std/sqrt(N))'), ylabel(['alpha averaged over ' int2str(size(alpha_uni,2)) ' experiments']);
xlabel('# of samples');
%%
