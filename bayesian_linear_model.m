...
for i=1:numFuncs
    mu_j=-limit/2+i/2;
    functions{i} = @(x) randn(size(x));%@(x) exp(-((x-mu_j).^2)/(2*s^2));
end
...
% dummy = zeros(1,11);
model.alpha(1:10) = 1;
model.w = model.alpha ;  %*eye(model.dimension)
% model.w
...
%% Sampling
numSamples = 20;
trainX = unifrnd(-2,2, [length(model.w) numSamples]);
targets= zeros(model.dimension, numSamples);
targetNoise = zeros(model.dimension, numSamples);
...
trainY = trainX'*model.w' +sqrt(1/model.beta)*randn(numSamples,1);%phi(functions, model.w, trainX);
